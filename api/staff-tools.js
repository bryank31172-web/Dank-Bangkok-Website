...
export default async function handler(req,res){
  ...
  try{
    ...
    if(section==="promotions"){
      ...
      if(action==="create") ov.added.push({id:"web-"+crypto.randomUUID(),name,category:text(b.category,80)||"Other",price:Math.max(0,number(b.price)),stock:Math.max(0,number(b.stock)),image:picture,picture,strain,strainType:cannabisType,type:cannabisType,description,gramOptions:grams,freeDelivery,discountEnabled,discountType,discountValue,available,_hidden:!available,_source:"staff"});
