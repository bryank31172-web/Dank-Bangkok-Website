...
/* ---- optional: push an online order into StoreHub as a transaction ----
   ...
   giftLines is what came out of the custom-box ledger (api/_boxgifts.js): the
   hash, brownie, jelly, Backwoods, papers, tee and hat that ride along free
   with every 28 grams. They are appended as ฿0 lines rather than being written
   to an inventory endpoint because this API exposes inventory read-only —
   GET /inventory/<storeId> and nothing to PUT back. A zero-priced line on the
   Sale is the only thing that moves POS stock from out here.

   Two things that must stay true. A gift with no StoreHub id mapped yet is
   skipped, not sent with productId "" — an unmapped tee shouldn't take a real
   transaction down with it. And the gifts add nothing to `total` or `subTotal`,
   so the numbers still reconcile against what the customer actually paid. */
export async function pushTransaction(order, orderId, giftLines = []) {
  ...
  const gifts = (Array.isArray(giftLines) ? giftLines : [])
    .filter((g) => g && g.shId && Number(g.qty) > 0)
    .map((g) => ({ productId: g.shId, quantity: Number(g.qty), unitPrice: 0, total: 0 }));
  if (gifts.length) items.push(...gifts);
  const body = {
  ...
  return { ok: r.ok, status: r.status, gifts: gifts.length };
}
